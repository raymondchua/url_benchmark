...
"""Humanoid Big Leg Domain. Leg is scaled down to 1.25 of the original size following gym-extensions.
...
def get_model_and_assets():
    ...
    xml = resources.GetResource(
        os.path.join(root_dir, "custom_dmc_tasks", "humanoidbigleg.xml")
    )
...
def stand(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
    ...
    task = HumanoidBigLeg(move_speed=0, pure_state=False, random=random)
...
def walk(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
    ...
    physics = Physics.from_xml_string(*get_model_and_assets())
    task = HumanoidBigLeg(move_speed=_WALK_SPEED, pure_state=False, random=random)
    environment_kwargs = environment_kwargs or {}
    return control.Environment(
        physics,
        task,
...
def run(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
    ...
    physics = Physics.from_xml_string(*get_model_and_assets())
    task = HumanoidBigLeg(move_speed=_RUN_SPEED, pure_state=False, random=random)
    environment_kwargs = environment_kwargs or {}
    return control.Environment(
        physics,
        task,
...
def run_pure_state(
    ...
    """Returns the Run task."""
    physics = Physics.from_xml_string(*get_model_and_assets())
    task = HumanoidBigLeg(move_speed=_RUN_SPEED, pure_state=True, random=random)
    environment_kwargs = environment_kwargs or {}
    return control.Environment(
        physics,
        task,
...
class HumanoidBigLeg(base.Task):
    """A humanoid task."""

    def __init__(self, move_speed, pure_state, random=None):
        """Initializes an instance of `HumanoidBigLeg`.
