...
class SFSimpleAgent(DDPGAgent):
    def __init__(self, update_task_every_step, sf_dim, num_init_steps, lr_task,
                 **kwargs):
        self.sf_dim = sf_dim
        ...
        self.task_params = nn.Parameter(torch.randn(self.sf_dim, requires_grad=True, device=self.device))
    ...
    def update(self, replay_iter, step):
        ...
        # update meta
        metrics.update(self.regress_meta_grad_descent(next_obs.detach(), task, reward, step))
    ...
    def update_critic(self, obs, action, reward, discount, next_obs, task, step):
        ...
        with torch.no_grad():
            ...
            dist = self.actor(next_obs, stddev)
            next_action = dist.sample(clip=self.stddev_clip)
            target_Q1, target_Q2, basis_features = self.critic_target(next_obs, next_action, task)
