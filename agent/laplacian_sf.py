...
class LaplacianSFAgent(DDPGAgent):
    def __init__(
    ...
    def update_critic(self, obs, action, reward, discount, next_obs, task, step):
        ...
        with torch.no_grad():
            ...
            dist = self.actor(next_obs, stddev)
            next_action = dist.sample(clip=self.stddev_clip)
            target_Q1, target_Q2, next_basis_features = self.critic_target(
        ...
        # add laplacian loss to critic loss
        critic_loss += laplacian_loss
